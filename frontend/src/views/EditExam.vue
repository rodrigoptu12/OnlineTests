    ...
    <div class="wrapper-headline flex flex-wrap -mx-3 mb-6">
      <div class="w-full px-3 mb-6 md:mb-0">
        ...
        <p class="text-lg font-normal text-gray-500 lg:text-l dark:text-gray-400 mb-4">
          Área de edição de exames.
        </p>
      </div>
    </div>
    <div class="w-full">
      <form class="w-full" @submit="editExam">
...
export default {
  ...
  methods: {
    ...
    axios.patch(`/exame/${this.$route.params.id}`, data)
        .then(() => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        this.titulo = examData.titulo; // Set the exam title
        this.inicio = examData.inicio; // Set the exam start time
        this.fim = examData.fim;       // Set the exam end time

        // Populate the list of selected questions with the fetched exam's questions
        this.list = examData.questoes.map(question => ({
